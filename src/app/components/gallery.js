...
export default function Gallery({openGal, lastSeen}) {
    ...
    return (
        <section className="md:w-2/4 md:px-14 flex items-center flex-col lg:pt-0 md:pt:10 w-screen p-0 m-0  ">
        ...
        <span  onClick={()=>prev()} className="h-fit self-center bg-white absolute top-96 left-2 rounded-full p-5 hover:cursor-pointer">
        ...
        <span onClick={()=>next()} className="h-fit self-center bg-white absolute top-96 right-1 rounded-full p-5 hover:cursor-pointer">
            ...
            <article className={!small ? "flex flex-wrap justify-between w-full mt-4" : "hidden"}>
